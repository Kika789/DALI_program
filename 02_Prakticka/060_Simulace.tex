...
\medskip
\noindent{\sbf Podmínky validace:}
\begitems
    * Data hodnotíme pouze vizuálně.
    * Srovnáváme kombinované, nepřímé a přímé osvětlení.
    * Hodnotíme tvarovou shodu isofot.
    * Intenzitu osvětlení hodnotíme s rezervou  10~{\pcent} až 15~{\pcent}.
\enditems
...
\ctable{lccc}{
{\sbf Typ osvětlení}  &    {\sbf Tabule} [lm] & {\sbf Lavice} [lm]  &   {\sbf CCT} [K]   \cr
Nepřímé      &     5 500       &   11 000       &       5 470         \cr
Přímé        &     2 500       &    5 000       &       5 470         \cr
}
...
Kombinované osvětlení podle měření \ref[svet_mapa] dosahuje maxima kolem 600 luxů,
což odpovídá simulovaným hodnotám obr.~\ref[val0].
Tvarově se obě data velmi dobře shodují, což potvrzuje správnost modelu.
...
je v modelu přesně zachyceno, což je důležité pro přesné řízení světelných scén.


Podobné výsledky byly získány také při porovnání měřených a simulovaných dat
u nepřímého osvětlení obr.~\ref[svet_mapa2] a obr.~\ref[val1]
a~přímého osvětlení \ref[svet_mapa3] a obr.~\ref[val2].
