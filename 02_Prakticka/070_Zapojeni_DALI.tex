...
pro nastavení jasu (viz obrázek \ref[ovl0]).
...
\begitems
    ...
    * Odloučení funkce zapnutí/vypnutí od regulace jasu umožňuje intuitivnější a~pohodlnější ovládání.
...
Schéma zahrnuje:
\begitems
    * Tlačítka pro zapnutí/vypnutí a regulaci jasu jsou připojena k konektoru JP1.
    K tlačítkům 5 a 6 jsou připojeny pull-down rezistory o hodnotě 10 kΩ, protože vstupní piny procesoru
    neposkytují pull-down rezistory pro dané vstupy.
...
\begitems
    * {\sbf Napájecí konektoru:} Konektor pro připojení 5V napájení.
...
% Pro řízení světel byla použitá řídící jednotka s mikroprocesor \glref{ESP32} WROOM
% (viz obrázek \ref[pripojeni]).
Pro řízení světel byla zvolena řídící jednotka s mikrořadičem \glref{ESP32} WROOM, který se vyznačuje vysokým výkonem,
...
\begitems
    * {\sbf set failure system = 180:} Nastaví úroveň jasu světelných zařízení při výpadku mikrokontroléru. Hodnota 180 odpovídá 70% jasu.
...
\begitems
    * {\sbf Funkce - add to group (číslo zařízení číslo skupiny):}
...
{\sbf Definování scén:}
\begitems
    * {\sbf Funkce: set scene (č. zařízení číslo scén hodnota jasu):}
...
\noindent První scéna - dva světelné prvky byly nastavené na intenzitu jasu 200 a dva zbylé byly vypnuté
...
\noindent Druhá scéna - dva světelné prvky byly nastavené na vyšší intenzitu jasu 200 a dva zbylé byly na nižší 120
...
\noindent Třetí scéna byla nastavena podobně.
...
\noindent Poté je zapotřebí sepsat nějaké shrnutí: funguje, komentář, dává to smysl to tam instalovat?
