...
\medskip \clabel[tlac]{Deska s návrhem optimalizovaného ovládání}
\picw=10cm \cinspic 03_Obrazky/Ovladani/tlacitka2.jpg
\caption/f Deska s návrhem optimalizovaného ovládání
\medskip
...
\medskip \clabel[ovl2]{Elektrické schéma zapojení tlačítek}
\picw=10cm \cinspic 03_Obrazky/Ovladani/buttons2.jpg
\caption/f Elektrické schéma zapojení tlačítek
\medskip
...
\begitems
    * {\sbf Napájecího konektoru:} Konektor pro připojení 5~V napájení.
...
\picw=10cm \cinspic 03_Obrazky/Ovladani/board2.jpg
...
Pro řízení světel byla zvolena řídicí jednotka s mikrokontrolérem \glref{ESP32} WROOM,
který se vyznačuje vysokým výkonem,
nízkým odběrem energie a~širokou škálou funkcí (viz obrázek \ref[pripojeni]).
Řídicí jednotka je připojena k~plošnému spoji tlačítek pomocí konektoru JP2.
...
\medskip \clabel[pripojeni]{Řídicí jednotka}
\picw=10cm \cinspic 03_Obrazky/rid_jed.jpg
\caption/f Řídicí jednotka
\medskip
...
\picw=10cm \cinspic 03_Obrazky/DALI_svetla.jpg
...
\begitems
    ...
    * {\sbf Testování funkčnosti tlačítek:}
        Reakce řídicí jednotky na stisknutí jednotlivých tlačítek byla ověřována pomocí
...
\begitems
    * {\sbf Propojení světel s řídicí jednotkou:} Vodiče sběrnice DALI byly připojeny k~příslušným pinům řídicí jednotky.
    * {\sbf Napájení řídicí jednotky:} Napájecí zdroje 12 V byly připojeny k~napájecím konektorům řídicí jednotky.
...
Jako vývojové prostředí pro programování řídicí jednotky lze použít:

\begitems
    * {\sbf PlatformIO:} PlatformIO je open-source vývojové prostředí pro vývoj softwaru pro mikrokontroléry.
        Nabízí programování v mnoha jazycích a~podporuje mnoho různých mikrokontrolérů~\cite[platformio_ide].
    * {\sbf Arduino IDE:} Arduino IDE je vývojové prostředí pro programování mikrokontrolérů z platformy Arduino.
        Umožňuje psát kód v jazyce C++~\cite[arduino_ide].
    * {\sbf ESP--IDF (Espressif IoT Development Framework):} ESP--IDF je oficiální vývojové prostředí pro mikrokontroléry ESP32.
        Používá jazyk C a~poskytuje širokou škálu funkcí pro vývoj softwaru~\cite[noauthor_espidf].
\enditems

Ukazuje se, že nejvhodnějším vývojovým prostředím pro programování řídicí jednotky je PlatformIO,
...
Pro vývoj řídicí jednotky bylo zvoleno vývojové prostředí PlatformIO, z důvodu, že byla
k dispozici jednoduchá knihovna pro komunikaci řídicí jednotky se světly pomocí protokolu DALI verze 1.
...
\sec Inicializace řídicí jednotky:
...
\picw=14cm \cinspic 03_Obrazky/esp32_wroom2.jpg
...
V programu byly definovaný následující porty pro řídicí jednotku:
...
\medskip\noindent
{\sbf Seznam příkazů pro práci s řídicí jednotkou:}

Funkce PrintHelp() vypíše seznam používaných příkazů v kódu řídicí jednotky.
...
%     • zapojila světla do řídicí jednotky
%     • a~zdroj 12~V
...
% a~zapojila v~laboratorních podmínkách. Jako řídicí jednotka byla zvolena ESP3 (viz dokumentace).
...
\begitems
    * {\sbf set failure system (180):}
        Nastaví úroveň jasu světelných zařízení při výpadku řídicí jednotky.
        Hodnota 180 přibližně odpovídá {70~\pcent} maximálního výkonu.
    * {\sbf set power on (200):} Nastaví úroveň jasu při zapnutí zařízení.
        Hodnota 200 přibližně odpovídá {80~\pcent} maximálního výkonu.
    * {\sbf set max level (254):} Nastaví maximální dosažitelný jas.
        Hodnota 254 odpovídá {100~\pcent} maximálního výkonu.
    * {\sbf set min level (85):} Nastaví minimální dosažitelný jas.
        Hodnota 85 přibližně odpovídá {34~\pcent} maximálního výkonu.
...
\picw=6.5cm \cinspic 03_Obrazky/Ovladani/01a6.jpg
\caption/f Scéna 1
\medskip
...
\picw=6.5cm \cinspic 03_Obrazky/Ovladani/02b6.jpg
\caption/f Scéna 2
\medskip

\medskip
\noindent{\sbf Třetí scéna} - každý světelný prvek byl nastaven na jinou hodnotou jasu:
...
\noindent {\sbf Obsluha tlačitek pro regulaci jasu}
...
% \medskip\noindent
% {\sbf Inicializace řídicí jednotky:}
...
Cílem této části praktické části bylo navrhnout intuitivní ovládání
...
% Zvolili jsme tři tlačítka, kterými se bodou zapínat konkrétní světelné scény.
% Nejdříve byla ověřována funkčnost celé řídíci jednotky i s novými tlačítky.
% Po úspěšném testování byly implementovány scény pomocí příkazových funkcí.

% V laboratorních podmínkách jsme měli k dispozici čtyři světelná zařízení
% se kterými probíhala komunikace protokolem DALI.

% V průběhu testu su ukázalo, že zvolené řešení je jednodušší pro uživatele.
% Nedocházelo k náhodnému zapnutí nebo vypnutí světel, když bylo
% zapotřebí změnit hodnotu jasu, což se často stávalo právě
% při použití předchozího ovládacího panelu.
% Take uživatel jednoznačně ví, jak vypnout světla, když se už nevyužívají.

% Ukazuje se že zvolené řešení je vhodné pro uživatele,
% kteří mohou snadno ovládat světelný systém pomocí tlačítek
% protože už na první pohled je přehlednější.
