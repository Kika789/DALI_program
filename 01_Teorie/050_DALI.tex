...
Postupem podle přílohy A.1 \cite[iec101] se určí zařízení s nejnižší dlouhou adresou a té se přiřadí
volná krátká adresa například 0. Postup se opakuje pro další zařízení s následujícími dlouhými adresami.

Postup nastavení krátkých adres:
\begitems
    * Přepnutí do inicializačního režimu příkazem INITIALISE.
    * Nastavení dlouhé adresy pro vyhledávání nejnižší adresy, příkazy SEARCHADDRH, SEARCHADDRM, SEARCHADDRL a COMPARE.
    * Přiřazení krátké adresy příkazem PROGRAM SHORT ADDRESS.
...
zařízení, jejichž dlouhá adresa je menší nebo rovna hodnotě SEARCHADDR.
...
Použijeme příkaz SET SHORT ADDRESS.

\medskip \noindent
{\sbf Identifikace zařízení}

Pokud máme u všech zařízení nastaveny krátké adresy, můžeme začít s identifikací.
Použijeme příkaz IDENTIFY DEVICE po kterém světlo začne blikat po dobu 10~s.

Je možné použít i jiné příkazy, například vypnutí světla příkazem OFF,
přepnutí na maximální úroveň RECALL MAX LEVEL a další.

Pokud nám přiřazené krátké adresy nevyhovují můžeme je změnit příkazem PROGRAM SHORT ADDRESS.
