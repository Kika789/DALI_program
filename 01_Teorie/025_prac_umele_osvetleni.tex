...
Viditelné světlo tvoří malou část celého elektromagnetického spektra.
...
\begitems
% * {\sbf Osvětlenost (intenzita osvětlení) [$\bi E$]}:
* {\sbf Osvětlenost (intenzita osvětlení) [E]}:
    1 lx (lux) -- Veličina udává,
...
\medskip
kde d{\it Φ} [lm] je světelný tok dopadající na plochu d{\it A} [m$^2$].
...
% *{\sbf Svítivost [$\bi I$]}:
*{\sbf Svítivost [I]}:
   1 cd (kandela) -- Veličina udává, kolik světelného toku {\it Φ} vyzáří světelný zdroj nebo svítidlo
    do jednotkového prostorového úhlu {\it Ω} v~určitém směru (obr.~\ref[lcx]).
\enditems

    $$ I={{{\rm d} \it Φ} \over {\rm d} {\it Ω}}  \quad , \eqmark $$

\medskip
kde d{\it Φ} [lm] je světelný tok vyzařovaný do elementárního prostorového úhlu d{\it Ω} [sr].

\begitems
% * {\sbf Světelný tok [$\bi Φ $]}:
* {\sbf Světelný tok [Φ]}:
    1 lm (lumen) --
    Světelný tok udává celkové množství světla vyzařované zdrojem do všech směrů (obr.~\ref[lcx]).
    Jde o~světelný výkon, který je posuzován z~hlediska lidského oka.
\enditems

\medskip \clabel[lcx]{Vztah mezi lm, cd a~lx}
\picw=8cm \cinspic 03_Obrazky/lumen_candela_lux2.jpg
...
% * {\sbf Jas [$\bi L$]}:
* {\sbf Jas [L]}:
    1 cd m$^{-2}$ (kandela na metr čtvereční)
    -- Jas je měřítkem pro vjem jasnosti svítícího nebo osvětlovaného povrchu.
% * {\sbf Měrný světelný výkon [$\bi η$]}:
* {\sbf Měrný světelný výkon [η]}:
     1 lm W$^{-1}$ (lumen na watt) --
...
% * {\sbf Teplota chromatičnosti [$\bi {T_c}$]}:
* {\sbf Teplota chromatičnosti [$\bf T_c$]}:
    1 K~(kelvin) -- Teplotou chromatičnosti zdroje je označována ekvivalentní
    teplota tzv. černého zářiče (Planckova), při které je spektrální složení záření těchto dvou zdrojů blízké.
% * {\sbf Index barevného podání [$\bi Ra$]}:
* {\sbf Index barevného podání [Ra]}:
...
\secc Norma ČSN EN 12464-1

\noindent {\sbf Celým názvem: Norma ČSN EN 12464-1 Světlo a~osvětlení - Osvětlení pracovišť - Část 1: Vnitřní pracoviště}

\noindent
Norma stanovuje požadavky na osvětlení pro vnitřní pracoviště z~hlediska zrakové
pohody a~zrakového výkonu osob s~normálním nebo korigovaným zrakem \cite[CSN12464_1].

\medskip
\noindent V této části jsou uvedena některá doporučení z normy.
...
\medskip\noindent Doporučené hodnoty činitele odrazu povrchů:
\begitems
* strop: 0,7 až 0,9,
* stěny: 0,5 až 0,8,
* podlaha: 0,2 až 0,6,
...
500 -- 750 -- 1~000 -- 1~500 -- 2~000 -- 3~000 -- 5~000 -- 7~500 -- 10~000.}
...
\tskip 4pt
    ...
    300           &    520 -- 560    \cr
    200           &    570 -- 590    \cr
    $\leq$150     &    stejná jako pro místo zrakového úkolu       \crl
}
\caption/t Vztah mezi osvětlenostmi bezprostředního okolí a~osvětlenostmi místa zrakového úkolu \cite[CSN12464_1]
\endinsert
...
* Světlo v~bezprostředním okolí zrakového úkolu může být slabší než světlo nad samotným úkolem,
  ale nesmí klesnout pod hodnoty uvedené v tabulce \ref[vztah].
\enditems
...
\noindent
Pro posouzení rovnoměrnosti osvětlení je definován následující vztah:

$$ U = { E_{min} \over E_{m}} \quad , \eqmark $$

kde $E_{min}$ je minimální hodnota udržované osvětlenosti [lx]
a~$E_{m}$ je střední hodnota udržované osvětlenosti [lx].
...
\noindent {\sbf Specifické požadavky pro vzdělávací zařízení}

\noindent Druh místa zrakového úkolu/činnosti: Učebny -- obecné činnosti (viz. tab.~44~\cite[CSN12464_1])
\begitems
* Osvětlení musí být regulovatelné pro různé činnosti a~nastavení scény.
* U učeben pro malé děti může být osvětlenost 300~lx.
* Požadovaná osvětlenost 500~lx.
\enditems

\noindent Druh místa zrakového úkolu/činnosti: Prezentace pomocí projektorů a~chytrých tabulí (viz. tab.~44~\cite[CSN12464_1])
\begitems
* Osvětlení musí být regulovatelné.
* Je nutné zabránit zrcadlovým odrazům.
* Svislá osvětlenost 200~lx za (okolo) plátnem.
* Musí se zabránit přímému osvětlení plátna při prezentaci.
\enditems



\secc Norma ČSN 73 0580-3

\noindent Tato norma stanovuje požadavky na denní osvětlení škol.

\medskip
\noindent Školské stavby jsou děleny na:
\begitems
* předškolní zařízení,
* základní školy,
* střední školy.
...
Zároveň se vylučuje pruh o~šířce 500 mm od stěn, aby se minimalizovaly odrazy~\cite[CSN73_0580_3].
...
aby denní světlo přicházelo převážně ze strany nebo shora\cite[CSN73_0580_3].
