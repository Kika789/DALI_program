...
\begitems
    ...
    *{\sbf Typy příkazů:}
    \begitems\style o
        ...
        *{\sbf Dotazy:} Zjištění konfigurace světel, aktuálního jasu, hlášení poruch zařízení.
    \enditems
\enditems
...
který rozšiřuje původní verzi \glref{DALI} verze~1.
...
Norma \glref{DALI} verze~1 neumožňovala připojení těchto zařízení přímo na sběrnici.
...
\begitems
    *{\sbf Single master:} Tyto jednotky jsou kompatibilní s \glref{DALI} verze~1 a~slouží jako hlavní řídicí prvek v systému.
...
Single master má povolenou odchylku od nominální hodnoty doby půl bitu 416,7 µs až o 10{ \pcent},
zatímco multi master pouze o 5{ \pcent}.
...
Zařízení v systému \glref{DALI} se dají propojit několika způsoby~\cite[iec101] (obr.~\ref[topolgie]):
\begitems
    * {\sbf Hvězdicová topologie:} Kabely od všech zařízení jsou vedeny centrálně k~jednomu zařízení.
    * {\sbf Sériová topologie:} Zařízení jsou propojena v řadě, jedno za druhým.
    * {\sbf Kombinace obou:} Může se kombinovat hvězdicová a~sériová topologie.
...
\picw=7cm \cinspic 03_Obrazky/g38.png
...
Standard IEC 62386~101~\cite[iec101] specifikuje elektrické parametry sběrnice v systému \glref{DALI}. Nejdůležitější z nich jsou:
\begitems
    *{\sbf Polarita:} Rozhraní je nezávislé na polaritě napětí s výjimkou případů, kdy je napájecí zdroj integrován do sběrnice.
    *{\sbf Značení:}  Datové vodiče by měly být označeny "da" nebo "DA".
    ...
    *{\sbf Napětí:} Rozhraní má dvě úrovně napětí: nízkou a~vysokou.
    ...
    \enditems
    *{\sbf Odběr:} Každé zařízení připojené k~sběrnici by mělo v~klidovém stavu maximálně odebírat 2~mA.
    *{\sbf Proud:} Maximální povolený proud na sběrnici je 250~mA.
    *{\sbf Aktivní stav:} Při nízké úrovni napětí vysílače a~odběru 250 mA nesmí výstupní napětí
    překročit 4,5~V (tab.10 \cite[iec101]).
    *{\sbf Hrana signálu:} Náběžná a~sestupní hrana signálu, by neměla být kratší než 3 µs,
           aby se minimalizovalo rušení  \glref{EMI}.
\enditems
...
Přenosový protokol \glref{DALI} využívá k~přenosu informace hrany signálu
s~pevně stanovenou přenosovou rychlostí 1200 bps.
Logické jedničce odpovídá vzestupná hrana signálu a~nule odpovídá sestupná hrana signálu (viz obr.~\ref[kodovani]).
...
Tento typ kódování je znám jako protokol Manchester~\cite[manchester_code].


\medskip \clabel[kodovani]{Kódování bitů v protokolu \glref{DALI}}
\picw=8cm \cinspic 03_Obrazky/logika.png
\caption/f Kódování bitů v protokolu \glref{DALI} \cite[bus_communication]
\medskip
...
\noindent
Kromě logických nul a~jedniček obsahuje každý datový rámec start a~stop bity,
které slouží k~synchronizaci komunikace (viz obr.~\ref[paket]):
\begitems
...
\noindent
Datový rámec obsahuje \cite[iec101]:
\begitems
* {\sbf Start bit}
...
Datové pakety protokolu \glref{DALI} se dělí na \cite[iec101]:
\begitems
 ...
 * norma IEC 62386~\cite[iec101]\cite[iec102]\cite[iec103].
\enditems
...
Celkový počet adres na jedné sběrnici je~64 pro předřadníky a~64 pro řídicí jednotky,
celkem 128 zařízení~\cite[iec101]\cite[iec102]\cite[iec103].
...
\noindent Postup nastavení krátkých adres:
\begitems
...
K tomuto účelu slouží příkaz "PROGRAM SHORT ADDRESS"~\cite[iec102].
...
světla na hodnoty uložené v dané scéně~\cite[iec102].
...
\begitems
    ...
    * K mikropočítači se sběrnice připojuje přes optické oddělovače realizované optočleny OK1 a~OK2~\cite[PC817].
\enditems
...
\begitems
    * Po spuštění mikropočítače se nastaví výstupní port na 1, čímž uvede sběrnici do klidového stavu.
\enditems
